...
public class GrilleImpl implements Grille {
    ...
    public void setValue(int x, int y, ElementDeGrille value) throws HorsBornesException, ValeurImpossibleException, ElementInterditException, ValeurInitialeModificationException {
        ...
        if (value != null && !isPossible(x, y, value)) {
            throw new ValeurImpossibleException("Impossible de placer cette valeur à cette position");
        }
    ...
    public boolean isPossible(int x, int y, ElementDeGrille value) throws HorsBornesException, ElementInterditException {
        ...
        for (int i = 0; i < taille; i++) {
            if ((grille[x][i] != null && grille[x][i].equals(value)) || (grille[i][y] != null && grille[i][y].equals(value))) {
                return false;
            }
        }
        ...
        for (int ligne = debutBlocX; ligne < debutBlocX + tailleBloc; ligne++) {
            for (int colonne = debutBlocY; colonne < debutBlocY + tailleBloc; colonne++) {
                if (grille[ligne][colonne] != null && grille[ligne][colonne].equals(value)) {
                    return false;
                }
            }
        }
